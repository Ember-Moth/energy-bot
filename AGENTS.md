...
- 全异步;非 Windows 平台事件循环为 uvloop(`main.py` 的 `_loop_factory`)
- 配置从工作目录的 `config.yaml` 读取(模板 `config.example.yaml`),不使用环境变量 / .env

## 常用命令

```bash
uv sync                    # 安装依赖
uv run energy-bot          # 启动(需 config.yaml 与公网 HTTPS)
...
```
src/energy_bot/
├── main.py        # 入口:Bot/Dispatcher + aiohttp webhook 服务,生命周期见 on_startup/on_shutdown
